...
(function ($) {
    ...
    function getDigitEAN(code, type) {
        ...
        if (type === "ean8") {
            ...
            for (i = 1; i < 7; i++) {
                result +=
                    encodingEAN[intval(code.charAt(i))][
                        intval(seq.charAt(i - 1))
                    ];
            }
    ...
    var getDigitDatamatrix = (function () {
        ...
        function selectIndex(dataCodeWordsCount, rectangular) {
            ...
            if (
                (dataCodeWordsCount < 1 || dataCodeWordsCount > 49) &&
                rectangular
            ) {
                return -1;
            }
                ...
                for (j = 0; j < totalColsCW; j++) {
                    if (i % (rowsRegionCW + 2) === 0) {
                        ...
                        datamatrixTemp[i + 1][j + 1] =
                            datamatrix[
                                i - 1 - 2 * parseInt(i / (rowsRegionCW + 2), 10)
    ...
    function digitToBmpRenderer($container, settings, digit, hri, mw, mh) {
        ...
        object.setAttribute(
            "data",
            "data:image/bmp;base64," + base64Encode(bmp)
        );
